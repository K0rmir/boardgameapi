...
boardGamesRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const [reqParam, value] of Object.entries(req.query)) {
      if (!validFilters.has(reqParam)) {
        res
          .status(400)
          .send(
            `Could not return results. There could be an issue with your query param, '${reqParam}'.`
          );
        logger(
          req,
          res,
    ...
    try {
      const boardgames = await BoardGameService.findBoardgames(
        filters,
        page,
        limit
      );
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const boardgame: BoardGame[] | string =
        await BoardGameService.findGameByTitle(gameName);
